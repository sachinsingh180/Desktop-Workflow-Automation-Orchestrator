...
device_index = int(input("type device number: "))
device_info = sd.query_devices(device_index)
sr_to_use = int(device_info['default_samplerate'])
print(f"Using samplerate: {sr_to_use}")

print(f"Recording 3 seconds on device {device_index}... speak loudly!")
recording = sd.rec(
    int(3 * sr_to_use),
...
sd.wait()
print(f"Max amplitude: {np.abs(recording).max()}  (1000+ when mic is correct)")