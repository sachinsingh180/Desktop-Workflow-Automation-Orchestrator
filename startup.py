...
recognizer.pause_threshold = 0.8  # less pause and silence= faster response
...
def play_song(query):
    ...
    if video_id:
        ...
        speak("Direct video not found, open the search.")
        log_command(query, "youtube_search", "fallback")


def open_website(name, url):
    if open_url_in_brave(url):
        speak(f"{name} opening it.")
    else:
        speak("Brave browser not found.")
...
def toggle_playback(action):
    ...
    try:
        ...
        speak(f"Song {action} finished.")
    except Exception as e:
        speak("error got to control it.")
...
def control_volume(direction, steps=3):
    ...
    try:
        for _ in range(steps):
            pyautogui.press(key)
        speak(f"Volume {'increase' if direction == 'up' else 'decrease'} done.")
    except Exception as e:
        speak("i got an errror to volume control.")
...
def control_brightness(direction, step=15):
    ...
    if current is None:
        speak("Brightness control not happen by laptop.")
        return
    ...
    if set_brightness(new_level):
        ...
        speak("i got an error in brightness.")
...
def execute_command(query):
    ...
    if not index_ready.is_set():
        speak("Indexing not completed yet, wait.")
    ...
    if path:
        try:
            ...
            speak("i got an error to open the app.")
            log_command(query, name, f"error:{e}")
    else:
        speak("App not found, sorry.")
...
def main():
    ...
    print("(Indexing running on background, now you can speak)")
    ...
    while True:
        ...
        if 'exit' in cmd or 'stop jarvis' in cmd or 'jarvis stop' in cmd:
            speak("Goodbye!")
            break
        ...
        if 'brightness' in cmd:
            if any(w in cmd for w in ('up', 'increase', 'increase', 'increase')):
                control_brightness('up')
            elif any(w in cmd for w in ('down', 'decrease', 'decrease', 'decrease')):
                control_brightness('down')
            else:
                speak("tekk me what i do brightness increase or decrease, speak.")
        elif 'volume' in cmd:
            if any(w in cmd for w in ('up', 'increase', 'increase', 'increase')):
                control_volume('up')
            elif any(w in cmd for w in ('down', 'decrease', 'decrease', 'decrease')):
                ...
                speak("tell me what i do volume increase or decrease, speak.")
            ...
            if cmd.startswith('debug'):
                ...
                speak(f"{len(matches)} matches found, see on console.")
            else:
                play_song(cmd)
            ...
            speak("cant understand. 'open' speak apps name.")
